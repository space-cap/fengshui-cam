...
<head>
  ...
  <!-- Google Fonts -->
  <link
    href="https://fonts.googleapis.com/css2?family=Noto+Serif+KR:wght@400;600;700&family=Noto+Sans+KR:wght@300;400;500&display=swap"
    rel="stylesheet">
  <!-- Bootstrap Icons -->
...
<body>
      ...
      <div class="main-card">
        ...
        <form id="analysis-form" enctype="multipart/form-data" method="post">
          ...
          <div class="upload-zone" id="upload-zone">
            ...
            <input type="file" id="room-image" name="room_image" accept="image/jpeg,image/png,image/webp,image/gif"
              style="position:absolute;inset:0;opacity:0;cursor:pointer;" aria-label="방 사진 업로드">
          </div>
  ...
  <script>
    ...
    const MAX_SIZE_MB = 15; // 모바일 카메라 원본 대비 넉넉하게 확장 (JS에서 압축하므로 무관)
    ...
    function compressImage(file, maxWidth, maxHeight, quality, callback) {
      ...
      reader.onload = function (event) {
        ...
        img.onload = function () {
          ...
          // JPEG 포맷으로 변환 (파일 크기 대폭 감소)
          canvas.toBlob((blob) => { callback(blob); }, 'image/jpeg', quality);
        };
      };
    }

    // ── 폼 submit → fetch()로 OpenAI API 호출 (Phase 4 + JS 압축)
    document.getElementById('analysis-form').addEventListener('submit', function (e) {
      ...
      // 이미지를 화면에서 즉시 800x800 급으로 줄이고 70% 화질로 세팅 (1~2초 만에 백업)
      compressImage(file, 800, 800, 0.7, function (compressedBlob) {
